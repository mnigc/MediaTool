...
export default function VideoCropPanel({
  ...
  params: CropParams;
  onChange: (p: CropParams) => void;
}) {
  const { t } = useI18n();
  const set = (patch: Partial<CropParams>) => onChange({ ...params, ...patch });
  return (
    <div className="space-y-3">
      <Field label={t("opt.cropMode")}>
        <select
          className={sel}
          value={params.mode}
          onChange={(e) => set({ mode: e.target.value as CropParams["mode"] })}
        >
      ...
      {params.mode === "center" ? (
        <Field label={t("opt.aspect")}>
          <select
            ...
            onChange={(e) => set({ aspect: e.target.value as CropParams["aspect"] })}
          >
            {ASPECTS.map((a) => (
