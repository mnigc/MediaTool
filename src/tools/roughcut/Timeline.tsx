...
function Filmstrip({
  ...
  // proportionally less timeline time. The 8px floor keeps slots from turning
  // into sub-pixel slivers at overview zoom without misrepresenting the strip.
  const slotW = Math.max((sourceDur / frames.length / speed) * pxPerSec, 8);
  return (
...
export default function Timeline({
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-white ring-1 ring-neutral-200 dark:bg-neutral-900 dark:ring-neutral-800">
      {header && (
        <div className="flex items-center gap-1 border-b border-neutral-100 px-1.5 py-1 dark:border-neutral-800">
          {header}
        </div>
      )}
      {/* Fixed outer height: ruler (h-5) + track (h-16) + the always-shown
          scrollbar (overflow-x-scroll) — sized so the track sits flush on the
          bar with nothing left over, and the card never resizes. */}
      <div className="h-[94px]">
        <div
          ref={scrollRef}
          className="h-full overflow-x-scroll overscroll-x-contain px-0 select-none"
        >
          {/* the scrub/press handler lives on the content, not the scroll box:
              the scrollbar belongs to the scroll element, so a pointerdown
              there would seek the playhead */}
          <div style={{ width }} className="relative" onPointerDown={onPointerDown}>
            {/* ruler */}
